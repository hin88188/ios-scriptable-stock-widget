...
- 💧 **量比分析** - 冷→熱漸變色彩顯示成交量變化
- 📊 **智能成交額線條** - 使用實際最大成交額作為 100% 基準，保證小額標的也有可見線條
- ⚡ **高效快取** - 排行榜 / 自選 / K 線三種快取，提升載入速度
- 🌙 **深淺模式** - 自動適配 iOS 深淺色主題
- 🔧 **多輪嘗試機制** - 智能修正 ST / ETF / 備用 Counter ID，支援新標的
- 🚀 **動態並發控制** - 根據股票數量自動調整併發數
- 🏗️ **模組化 & API 客戶端架構（v2.6）** - 新增 `LbkrsClient` 抽象 Lbkrs API，集中管理 URL 與 indicators，減少硬編碼、強化可維護性
