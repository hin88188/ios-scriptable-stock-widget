...
### v2.6-LbkrsClient 主要變更 (2025-11-08)
- 🏗️ **抽象 API 存取層**:
  - 新增 `LbkrsClient`，集中管理 Lbkrs 排行榜與個股 Detail API 的:
    - BASE URL
    - RANKING_PATH / DETAIL_PATH
    - 預設 `indicators[]`、`sort_indicator`、`order`、`offset`、`limit`
  - `Widget` 主流程與上層函式不再直接拼 URL。
- 🔧 **DataFetcher 職責收斂**:
  - 保留重試、超時、標頭設定與基礎 HTTP。
  - 新增 `fetchJson()`，統一 JSON 解析與錯誤記錄，供 `LbkrsClient` 使用。
- 🧩 **呼叫點重構（行為維持不變）**:
  - `fetchRanking()` 改為調用 `LbkrsClient.getRankingList()` + `StockDataMapper.fromRankingAPI()`。
  - `fetchWatchlist()` 使用 `LbkrsClient` + `tryFetchStock()` 取得 Detail，再用 `StockDataMapper.fromDetailAPI()`。
  - `tryFetchStock()` 改以 `LbkrsClient.getDetailByCounterId()` 多輪嘗試 ST/ETF。
  - `enrichData()` 改用 `LbkrsClient` 傳入 `KlineDataProcessor.fetch()`，並透過 `fetchLbkrsDetailData` 相容舊介面。
- 🧱 **結構優化**:
  - API endpoint 與查詢參數集中於 `LbkrsClient`，避免多處硬編碼。
  - 維持 `StockDataMapper`、`CounterIdHelper`、各 Cache 類與 UI 邏輯不變，實現漸進式重構。
- 🧪 **相容性**:
  - 舊的資料格式與視覺呈現 (包含 v2.5 EnhancedBars 的成交額線條) 完整保留。
  - 僅更動 API 呼叫與 URL 組裝方式，功能與輸出與 v2.5 一致。
...
**文件版本**: 2.6-LbkrsClient
**最後更新**: 2025-11-08
**適用對象**: AI 程式碼生成工具、開發者
**維護狀態**: ✅ 已驗證與 Widget-v2.6-LbkrsClient.js 完全同步